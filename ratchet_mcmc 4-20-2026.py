...
def mcmc(sig, n=int(2e5), seed=42):
    ...
    for i in range(n):
        ...
        if nJ is not None and abs(nJ) > 2.0:
            print(f"\n  *** J/P_0 = {nJ:+.8f} > 2 at step {i} (sigma={sig}) — dumping profile and exiting ***")
            fig_exit, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 7), sharex=True)
            ax1.plot(x, nV, color="darkred", lw=1.8)
            ax1.axhline(0, color="k", lw=0.5, ls=":")
            ax1.set_ylabel("V(x)", fontsize=12)
            ax1.set_title(f"Profile at J/P₀ = {nJ:+.8f}  (σ={sig}, step={i})", fontsize=13)
            ax1.grid(True, alpha=0.3)
            ax2.plot(x, nf, color="steelblue", lw=1.8)
            ax2.axhline( sig, color="k", lw=0.6, ls="--", label=f"±σ={sig}")
            ax2.axhline(-sig, color="k", lw=0.6, ls="--")
            ax2.axhline(0, color="k", lw=0.4, ls=":")
            ax2.set_ylabel("f(x) = -V'(x)", fontsize=12)
            ax2.set_xlabel("x", fontsize=12)
            ax2.legend(fontsize=10)
            ax2.grid(True, alpha=0.3)
            fig_exit.tight_layout()
            outpath = f"/Users/Adele/Downloads/Systematic Ratchet Current/J_exceeded_2_sig{sig}_step{i}.png"
            fig_exit.savefig(outpath, dpi=150)
            print(f"  Saved profile plot: {outpath}")
            txtpath = outpath.replace(".png", ".txt")
            with open(txtpath, "w") as fh:
                fh.write(f"# J/P0={nJ:+.8f}  sigma={sig}  step={i}\n")
                fh.write("# a_s coefficients (modes 0..K)\n")
                fh.write(" ".join(f"{v:.10e}" for v in ns) + "\n")
                fh.write("# a_c coefficients (modes 0..K)\n")
                fh.write(" ".join(f"{v:.10e}" for v in nc) + "\n")
                fh.write("# x  V  f\n")
                for xi, Vi, fi in zip(x, nV, nf):
                    fh.write(f"{xi:.8e}  {Vi:.10e}  {fi:.10e}\n")
            print(f"  Saved coefficients: {txtpath}")
            sys.exit(0)
        if nJ is not None:
            dE=abs(nJ)-abs(Jc)
...
fig.savefig(f"/Users/Adele/Downloads/Systematic Ratchet Current/ratchet_trajectories_HMCMC April{_tag}.png",
