...
Fourier-parameterized V(x), fixed roughness sigma^2 = (1/L) int (V')^2 dx.
Four regimes compared.

Sign of perturbative formula verified against direct ODE solution.
...
#returns the mean squared forcing (roughness) for given Fourier coefficients.
def rough(a_s, a_c): return 0.5*np.sum(omega**2*(a_s**2+a_c**2))
...
#renomarlize the Fourier coefficients to have the desired roughness sigma^2 = (1/L) int (V')^2 dx.
def proj(a_s, a_c, sig):
    a_s, a_c = enforce_bcs(a_s, a_c)
    r = rough(a_s, a_c)
    if r < 1e-30: return a_s, a_c
    s = sig/np.sqrt(r); return a_s*s, a_c*s
...
results = {}
for sig, ns, fm in configs:
    bs,bc,Jt,bt = mcmc(sig, ns, seed=137, fmax=fm)
    V,f,fp = build(bs,bc)
    Je = J_exact(V,f,fp,fm)
    J3 = J_p3(V,f); J5 = J_p5(V,f)
    ode = ode_validity(V, f)
    results[sig] = dict(bs=bs,bc=bc,Jt=Jt,bt=bt,V=V,f=f,fp=fp,
                        Je=Je,J3=J3,J5=J5,fmax=fm,ode=ode)
    mf = np.max(np.abs(f))
    ...
    Je_str = f"{Je:+.10f}" if Je is not None else "None (|f|>=fmax)"
...
fig.suptitle("Ratchet Current Optimization via MCMC\n"
             "Exact steady-state formula  |  Run-and-tumble, $\\alpha=v=1$, period $L=2\\pi$",
             fontsize=15, fontweight="bold", y=0.99)
gs = GridSpec(3, 4, figure=fig, hspace=0.35, wspace=0.3)

cols = {0.03:"#16a34a", 0.10:"#2563eb", 0.25:"#f59e0b", 0.30:"#dc2626"}
slbl = {s: f"$\\sigma={s}$" for s in cols}

for col, sig in enumerate([0.03, 0.10, 0.25, 0.30]):
    r=results[sig]; c=cols[sig]
    
    ax = fig.add_subplot(gs[0, col])
    ax.plot(x, r["V"], color=c, lw=1.8)
    ax.set_title(f"$V(x)$ — {slbl[sig]}", fontsize=10)
    ...
    ax.axhline(r["fmax"], color="k", lw=0.6, ls=":"); ax.axhline(-r["fmax"], color="k", lw=0.6, ls=":")
...
plt.savefig("/Users/Adele/Downloads/Systematic Ratchet Current/ratchet_trajectories_HMCMC April.png", dpi=150, bbox_inches="tight")
print("-> ratchet_main.png")

# ─── Figure 2: comparison table ──────────
fig2, ax2 = plt.subplots(figsize=(12, 4)); ax2.axis("off")
rows = []
for sig in [0.03, 0.10, 0.25, 0.30]:
...
fig2.tight_layout()
fig2.savefig("/Users/Adele/Downloads/Systematic Ratchet Current/ratchet_exact_v_perturbative_HMCMC April.png", dpi=150, bbox_inches="tight")
...
fig3.tight_layout()
fig3.savefig("/Users/Adele/Downloads/Systematic Ratchet Current/ratchet_density_HMCMC April.png",dpi=150,bbox_inches="tight")
...
fig4.tight_layout()
fig4.savefig("/Users/Adele/Downloads/Systematic Ratchet Current/ratchet_ode_residuals_HMCMC April.png",
