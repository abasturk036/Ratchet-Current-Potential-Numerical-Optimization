...
- Fix stochastic chain behaviour: windowed acceptance tracking, adaptive perturbation
  scale, unbiased pilot sampling in tune_temperature, smoother proposals via
  cumulative-sum random walk (avoids clipping-induced large jumps)
"""
...
class ParallelTemperingOptimizer:
    ...
    def compute_xi(self, V, a_idx, b_idx):
        ...
        integral = np.trapezoid(integrand[a_idx:b_idx+1], self.x[a_idx:b_idx+1])
    ...
    def compute_full_current(self, V):
        ...
        denominator_C = np.trapezoid(denom_C_integrand, self.x)
        ...
        # (1/L) ∫₀ᴸ exp(∫₀ˣ A dx') dx
        term1 = np.trapezoid(exp_cum, self.x) / self.L
        ...
        term2 = C * np.trapezoid(inner_integrals, self.x) / self.L
    ...
    def propose_perturbation(self, V_current, perturbation_scale=0.05):
        """
        Propose a new V by adding a smooth, spatially-correlated perturbation.

        Uses a cumulative-sum random walk (Brownian bridge) rescaled to the
        desired RMS amplitude, which avoids the large discontinuous jumps that
        point-wise iid noise creates after clipping.  The perturbation is
        zero-mean so the DC level of V does not drift.
        """
        max_attempts = 20

        for attempt in range(max_attempts):
            # Brownian-bridge perturbation: smooth and zero-mean
            noise  = np.random.randn(self.n_points)
            walk   = np.cumsum(noise)
            walk  -= np.linspace(walk[0], walk[-1], self.n_points)  # pin endpoints → bridge
            walk  -= walk.mean()                                      # enforce zero mean
            walk  /= (walk.std() + 1e-12)                            # unit RMS
            perturbation = perturbation_scale * walk
    ...
    def run_parallel_tempering(self, n_iterations=10000, V_init=None, 
                               ...
                               window=200):
        """
        Run parallel markov chains with fixed temperatures. Each chain runs
        independently at its own fixed temperature.

        Acceptance rates are tracked as a rolling windowed average over the
        last `window` steps so they reflect current chain behaviour rather than
        the cumulative rate (which decays monotonically to zero once the chain
        stalls).

        An adaptive perturbation scale is maintained per chain: every
        `window` steps the scale is nudged up or down by 10% to push the
        windowed acceptance rate toward the [10%, 20%] target band.
        """
        ...
        # Per-chain adaptive perturbation scale — start from the global default
        scale_chains = {T: perturbation_scale for T in self.temperatures}
        ...
        # Rolling acceptance window: circular buffer of 0/1 outcomes per chain
        accept_buffer = {T: np.zeros(window, dtype=float) for T in self.temperatures}
        buf_idx       = {T: 0 for T in self.temperatures}

        print(f"\n" + "="*80)
        print(f"PARALLEL HMMC")
        print(f"Running {self.n_chains} chains at fixed temperatures")
        print(f"Temperatures: {self.temperatures}")
        print(f"Gradient constraint: -1+ε < ∂V/∂x < 1-ε  (ε=0.1)")
        print(f"Acceptance tracked over rolling window of {window} steps")
        ...
        for i in range(n_iterations):
            ...
            for T in self.temperatures:
                V_chains[T], J_chains[T], accepted = self.metropolis_step(
                    V_chains[T], J_chains[T], T, scale_chains[T]
                )

                # Record outcome in rolling buffer
                accept_buffer[T][buf_idx[T] % window] = float(accepted)
                buf_idx[T] += 1

                if accepted and J_chains[T] > J_best:
                    # Update global best — deep copy to avoid aliasing
                    V_best = np.array(V_chains[T], copy=True)
                    J_best = J_chains[T]

            # Adapt perturbation scales and save history every `window` steps
            if (i + 1) % window == 0:
                for T in self.temperatures:
                    windowed_rate = accept_buffer[T].mean()

                    # Nudge scale toward 10-20% acceptance band
                    if windowed_rate < 0.10:
                        scale_chains[T] *= 0.85   # proposals too large → shrink
                    elif windowed_rate > 0.20:
                        scale_chains[T] *= 1.15   # proposals too small → grow
                    scale_chains[T] = np.clip(scale_chains[T], 1e-4, 2.0)
            ...
            if i % save_every == 0:
                for T in self.temperatures:
                    ...
                    # Use rolling windowed acceptance rate for history
                    windowed_rate = accept_buffer[T].mean()
                    self.acceptance_history[T].append(windowed_rate)
            
            # Progress report
            if (i+1) % 1000 == 0:
                print(f"\nIteration {i+1}/{n_iterations}:")
                for T in self.temperatures:
                    w_rate = accept_buffer[T].mean()
                    print(f"  T={T:.2e}: J={J_chains[T]:12.4f}, "
                          f"windowed acc={w_rate:.3f}, scale={scale_chains[T]:.4f}")
                print(f"  Best overall: J={J_best:.4f}")
                print("-"*80)
        
        print("\n" + "="*80)
        print(f"Best J/P₀ found: {J_best:.6f}")
        print(f"\nFinal windowed acceptance rates and scales:")
        for T in self.temperatures:
            print(f"  T={T:.2e}: acc={accept_buffer[T].mean():.3f}, "
                  f"scale={scale_chains[T]:.4f}")
        print("="*80)
    ...
    def tune_temperature(self, V_init=None, n_samples=500, perturbation_scale=0.1,
        ...
        Strategy: collect a pilot sample of delta_J values using an unbiased
        random walk (Metropolis at infinite temperature, i.e. always accept),
        then binary-search for the temperature T* such that the expected
        acceptance rate E[min(1, exp(delta_J / T*))] falls in
        [target_low, target_high].

        Using an always-accept pilot (rather than greedy uphill) gives an
        unbiased sample of the delta_J distribution, so T_opt is not inflated
        by systematically large positive jumps.
        ...
        # Warm up: take the first valid (non-pathological) proposal
        for _ in range(200):
        ...
        # Collect pilot delta_J using unbiased random walk (always accept)
        # so the sample reflects the full delta_J distribution, not just uphill moves
        delta_J_samples = []
        print("\nTuning temperature — collecting unbiased pilot samples...")
        for _ in range(n_samples):
            ...
            if np.isfinite(J_prop) and J_prop > -1e5:
                ...
                V_current = V_prop   # always accept (infinite-T pilot)
                J_current = J_prop
        ...
        # Binary search over temperature (geometric midpoint: T spans many decades)
        T_lo, T_hi = 1e-6, 1e12
        for _ in range(100):
    ...
    def plot_all_temperatures(self, V_init, V_best):
        ...
        for T in self.temperatures:
            iterations = np.arange(len(self.J_history[T]))
        ...
        # Plot 3: Windowed acceptance rates
        ax3 = fig.add_subplot(gs[2, 0])
        for T in self.temperatures:
            iterations = np.arange(len(self.acceptance_history[T]))
            ax3.plot(iterations, self.acceptance_history[T], '-',
                    color=temp_colors[T], linewidth=2, alpha=0.7, label=f'T={T:.2e}')
        
        ax3.axhline(y=0.10, color='k', linestyle='--', alpha=0.4, linewidth=1.5)
        ax3.axhline(y=0.20, color='k', linestyle='--', alpha=0.4, linewidth=1.5,
                    label='Target band (10-20%)')
        ax3.set_xlabel('Save index', fontsize=12)
        ax3.set_ylabel('Windowed Acceptance Rate', fontsize=12)
        ax3.set_title('Windowed Acceptance Rates (All T)', fontsize=13, fontweight='bold')
...
def main():
    ...
    oom  = 1e3
    temperatures = [0.01*oom, 0.1*oom, 0.5*oom, 1.0*oom, 2.0*oom]
