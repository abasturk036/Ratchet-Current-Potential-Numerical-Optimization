...
class ParallelTemperingOptimizer:
    ...
    def compute_full_current(self, V):
        ...
        if not self.check_gradient_constraint(V, epsilon):
            return 1  # the step is rejected if constraint violated
    ...
    def plot_gradient(self, V_init, V_best):
        """
        Plot final gradients for all temperatures (full width)
        """
        ...
        # Final gradients (check constraint)
        ax.axhline(y=-1, color='r', linestyle='--', alpha=0.5, linewidth=2, label='Constraint')
        ...
        for T in self.temperatures:
            V_final = self.V_history[T][-1]
            dV_final = self.compute_derivative(V_final)
            ax.plot(self.x, dV_final, '-', color=temp_colors[T],
                    linewidth=2, alpha=0.7, label=f'T={T:.2e}')
        
        ax.set_xlabel('Position x', fontsize=12)
        ax.set_ylabel('∂V/∂x', fontsize=12)
        ax.set_title('Final Gradients: -1 < ∂V/∂x < 1', fontsize=13, fontweight='bold')
        ax.legend(fontsize=9, ncol=6)
        ax.grid(True, alpha=0.3)
        ax.set_ylim(-1.2, 1.2)

        plt.tight_layout()
        plt.savefig('/Users/Adele/Downloads/Systematic Ratchet Current/HMMC_V4_gradients.png', 
                   dpi=300, bbox_inches='tight')
        print("\nGradient plot saved: HMMC_V4_gradients.png")
        return fig
...
def main():
    ...
    # Initialize with 5 temperatures spanning low to ~10^6 scale --> too high
