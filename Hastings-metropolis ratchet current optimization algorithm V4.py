...
Version 4 (modified 2/23/2026)
- Update temperature s.t. acceptance rate is 10 - 20%. This means that temperature values should  be on par with max J values
- update \alpha and v such that run length is much smaller than system size
- understand the details of the code
- implement full expression for xi
"""
...
class ParallelTemperingOptimizer:
    ...
    def compute_A_integrand(self, V):
        ...
        dV = self.compute_derivative(V)
        f = -dV
        df = jnp.gradient(f, self.dx)
        denom = 1.0 - f**2
        denom = jnp.where(jnp.abs(denom) < 1e-8,
                          jnp.sign(denom + 1e-15) * 1e-8, denom)
    ...
    def compute_cumulative_A(self, A):
        ...
        """
        cum = jnp.concatenate([jnp.array([0.0]),
                               jnp.cumsum(0.5 * (A[:-1] + A[1:]) * self.dx)])
    ...
    def compute_full_current(self, V):
        ...
        exp_cum = jnp.exp(jnp.clip(cum, -50.0, 50.0))
        ...
        denominator_C = jnp.trapezoid(denom_C_integrand, self.x)
        denominator_C = jnp.where(jnp.abs(denominator_C) < 1e-10,
                                  jnp.sign(denominator_C) * 1e-10,
                                  denominator_C)

        if not jnp.isfinite(denominator_C):
            return -1e10

        C = numerator_C / denominator_C

        if not jnp.isfinite(C):
            return -1e10

        # ---- First term of Eq. 13 denominator ----
        # (1/L) ∫₀ᴸ exp(∫₀ˣ A dx') dx
        term1 = jnp.trapezoid(exp_cum, self.x) / self.L
        ...
        b_over_exp = b / jnp.where(jnp.abs(exp_cum) < 1e-15, 1e-15, exp_cum)
        # Cumulative integral of b_over_exp from 0 to x_i
        cum_b_over_exp = jnp.concatenate(
            [jnp.array([0.0]),
             jnp.cumsum(0.5 * (b_over_exp[:-1] + b_over_exp[1:]) * self.dx)])

        inner_integrals = exp_cum * cum_b_over_exp
        term2 = C * jnp.trapezoid(inner_integrals, self.x) / self.L

        denominator_JP0 = term1 + term2

        if not jnp.isfinite(denominator_JP0) or jnp.abs(denominator_JP0) < 1e-12:
            return -1e10
    ...
    def propose_perturbation(self, V_current, perturbation_scale=0.05, n_modes=3):
        ...
        for attempt in range(max_attempts):
            ...
            # Deep copy via np to avoid JAX aliasing: always materialise a fresh numpy array
    ...
    def run_parallel_tempering(self, n_iterations=10000, V_init=None, 
        ...
        V_chains = {T: jnp.array(np.array(V_init, copy=True)) for T in self.temperatures}
        ...
        # Best overall — stored as numpy array to avoid JAX aliasing
    ...
    def plot_all_temperatures(self, V_init, V_best):
        ...
        ax6.set_xticklabels([f'T={T:.2e}' for T in self.temperatures], fontsize=10)
        ...
        ax7.set_title(f'Evolution at T={T_best:.2e} (Coldest Chain)', 
        ...
        plt.savefig('/Users/Adele/Downloads/Systematic Ratchet Current/HMMC_V4_results.png', 
    ...
    def plot_diagnostics_all_temps(self):
        ...
        plt.tight_layout()
        plt.savefig('/Users/Adele/Downloads/Systematic Ratchet Current/HMMC_V4_diagnostics.png',
                   dpi=300, bbox_inches='tight')
...
def main():
    ...
    temperatures = [0.01e2, 0.1e2, 0.5e2, 1.0e2, 2.0e2]
