...
L, N, K = 24*np.pi, 512, 16
dx = L/N; x = np.linspace(0, L, N, endpoint=False)
_tag = f"_L{L}_K{K}_N{N}"   # appended to every output filename
...
# ─── MCMC ────────────────────────────────
def mcmc(sig, n=int(2e5), seed=42, fmax=0.995):
    ...
    t0 = time()
    for i in range(n):
        '''sz=ss*(1-0.7*p)''' #annealed step size does not make too much sense here
        #p=i/n; beta=b0+(b1-b0)*p; sz = ss #static step size for simplicity, but could also anneal 
        kappa=1/n*np.log(b1/b0); p=i/n; beta=b0*np.exp(kappa*i); sz = ss
...
for sig, ns, fm in configs:
    bs,bc,Jt,bt = mcmc(sig, ns, seed=None, fmax=fm)
    V,f,fp = build(bs,bc)
...
for ax, sig in zip(axes5.flat, [0.03, 0.10, 0.25, 0.30]):
    r  = results[sig]
    bs = r["bs"]; bc = r["bc"]
